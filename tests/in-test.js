...
describe('in', ()=>{
  describe('main', ()=>{
    ...
    describe('when updates are available', ()=>{
      const update = {
        message: {text: 'hi'}, update_id: 42, chat: {
          username: 'carlo',
          id: 123
        }
      ...
      it('call writeFile with the update as string', async ()=>{
        ...
        const res = await main(readConfig, noop, noop, check, writeFileMock, dest)

        sinon.assert.calledWith(writeFileMock, `${dest}/message`, JSON.stringify(update))
      })

      describe('but all the message are filtered out', ()=>{
        const _err = console.error
        beforeEach(() => {console.error = sinon.spy()})
